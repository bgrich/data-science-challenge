...
### Removing `same_partner`, `new_lead_partner`

```{r}
var_remove <- modified_data_for_model %>% 
  select(-same_partner, -new_lead_partner)
...
var_remove_model_4 <- train(x = train_independent, 
                  y = train_dependent$booking_event, 
      method = "ranger", 
      trControl = trainControl(method = "repeatedcv", 
                               number = 10,
...
```{r}
rf_prediction_var_remove_4 <- predict(var_remove_model_4, testing[, -1])

cm_var_remove_4 <- confusionMatrix(rf_prediction_var_remove_4, testing$booking_event)
```

### Removing `same_partner`
...
var_remove_model_5 <- train(x = train_independent, 
                  y = train_dependent$booking_event, 
      method = "ranger", 
      trControl = trainControl(method = "repeatedcv", 
                               number = 10,
...
```{r}
rf_prediction_var_remove_5 <- predict(var_remove_model_5, testing[, -1])

cm_var_remove_5 <- confusionMatrix(rf_prediction_var_remove_5, testing$booking_event)
```

### Removing `new_lead_partner`

```{r}
var_remove <- modified_data_for_model %>% 
  select(-new_lead_partner)
...
var_remove_model_6 <- train(x = train_independent, 
                  y = train_dependent$booking_event, 
      method = "ranger", 
      trControl = trainControl(method = "repeatedcv", 
                               number = 10,
...
```{r}
rf_prediction_var_remove_6 <- predict(var_remove_model_6, testing[, -1])

cm_var_remove_6 <- confusionMatrix(rf_prediction_var_remove_6, testing$booking_event)
```
...
comparison_var_remove <- bind_rows(cm_under$byClass[column_names], 
                        ...
                        cm_var_remove_4$byClass[column_names],
                        cm_var_remove_5$byClass[column_names], 
                        cm_var_remove_6$byClass[column_names]) %>% 
...
Based on the many different models run, the under-sampled random forest 
provided the highest sensitivity and accuracy in identifying booking events. 
From the trained model, I can extract the importance of the different variables 
involved. The two variables that appear to have the largest effect on the 
models are the `same_partner` and `new_lead_partner` variables. By removing both 
of those variables from the model, the sensitivity dropped from 75% to 60% 
while keeping the same level of specificity. Looking at the comparison of the 
different models, it looks like overall removing the `new_lead_partner` variable 
had the single largest effect on the sensitivity. These variables are likely 
highly correlated which is why the big drop does not occur until both are 
